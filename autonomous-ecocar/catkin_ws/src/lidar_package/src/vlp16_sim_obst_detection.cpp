...
#include "std_msgs/Float32MultiArray.h"
double sensor_origin_x=0.0;
double sensor_origin_y=0.0;
//DLSH
#define LOOP_RATE 20 
...
void recieveLiDARDataCallback(const sensor_msgs::PointCloud2 &pkt)
{
  ...
  //Split floor
  floor_plane.filterDistance(laserpoints, MAX_GROUND_PLANE_DISTANCE, floorpoints); // this gives floorpoints
  ...
  for (size_t i = 0; i < clouds.size(); i++)
  {
    if(clouds.at(i)->size() >= 3){ // Need four points to make Hull
      if(cloud_extremes.at(i).z_max - cloud_extremes.at(i).z_min > MIN_OBSTACLE_HIGHT || cloud_extremes.at(i).z_max >= MIN_OBSTACLE_GOUND_DISTANCE){ // Filter before hulls are made
        pcl::PointCloud<pcl::PointXYZ>::Ptr flat_cloud (new pcl::PointCloud<pcl::PointXYZ>);

        // Project points onto plane
        if(PROJECT_ONTO_CURRENT_GROUND_PLANE){
...
void recieveOdoDataCallback(const std_msgs::Float32MultiArray::ConstPtr& msg_data){
  odo.update(
   ...
   sensor_origin_x=msg_data->data[0];
   sensor_origin_y=msg_data->data[1];
}
...
int main(int argc, char **argv)
{
 ...
 //DLSH ADD
    ros::Publisher pcl_pub = n.advertise<sensor_msgs::PointCloud2> ("pcl_output", 50);
    //ros::Publisher pcl_pub = n.advertise<sensor_msgs::PointCloud> ("pcl_output", 50);
    ...
    if(recieved_new_flag || KEEP_PUBLISHING){
      ...
      if(MAKE_VISUALIZATION_PLOTS){
        ...
        if(FIT_PLANE_TOO){
          ...
          grid_points->push_back(floor_plane.getXYPoint(5.0,5.0));
          grid_points->push_back(floor_plane.getXYPoint(5.0,-5.0));
          grid_points->push_back(floor_plane.getXYPoint(-5.0,5.0));
          grid_points->push_back(floor_plane.getXYPoint(-5.0,-5.0));
      ...
      //DLSH ADD
        lidar_package::obsts output_obsts=obstaclesToMsg(&obstacles, &odo, recieved_new_timestamp);
        int cloud_width=0;
        for(int index_test_i=0;index_test_i<output_obsts.vector_len;++index_test_i){
           cloud_width+=output_obsts.vector_obst[index_test_i].vector_len;
        }
        //cloud.sensor_origin_[0]=sensor_origin_x;
        //cloud.sensor_origin_[1]=sensor_origin_y;
        cloud.width=cloud_width;
        cloud.height=1;
        cloud.points.resize(cloud.width * cloud.height);
        int index_p=0;
        for(int index_test_i=0;index_test_i<output_obsts.vector_len;++index_test_i){
            for (int index_test_j = 0; index_test_j < output_obsts.vector_obst[index_test_i].vector_len; ++index_test_j) {
                cloud.points[index_p].x=output_obsts.vector_obst[index_test_i].vector_point[index_test_j].x;
                cloud.points[index_p].y=output_obsts.vector_obst[index_test_i].vector_point[index_test_j].y;
                cloud.points[index_p].z=0.0;
                ++index_p;
            }
        }
