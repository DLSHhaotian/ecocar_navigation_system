...
{	
	std::ofstream logfile;	// TODO For Logging
	...
	bool PIKSI::PIKSIOpenNoLock( )
	{
		...
		char filename[100];
		sprintf(filename, "/home/dynamo/log_files/gps_fix_log_%d.txt", (int)ros::WallTime::now().toSec() - 1494796385);
    	logfile.open(filename);
		logfile << "Time,Latitude,Longitude,Height,HDOP,NSats,Status\n";
...
//      sbp_register_callback(&state, SBP_MSG_BASELINE_HEADING, &baseline_heading_callback, (void*) this, &baseline_heading_callback_node);
...
//		sbp_register_callback(&state, SBP_VEL_ECEF, &vel_ecefCallback, (void*) this, &vel_ecef_callback_node);
		...
		imu_pub = nh.advertise<sensor_msgs::Imu>("gps/imu_raw", 1);
	...
	void pos_llh_callback(u16 sender_id, u8 len, u8 msg[], void *context)
	{
		...
		/* 	Status (llh.flags)
			...
			4	Fixed RTK
		*/
		llh_msg->status.status = (unsigned int)llh.flags;
    	...
    	logfile << std::setprecision(12) << ros::Time::now() << "," << llh.lat << "," << llh.lon << "," << llh.height << "," << driver->hdop << "," << driver->num_llh_satellites << "," << (unsigned int)llh.flags << "\n";
		ROS_INFO_STREAM("GPS llh: " << std::setprecision(12) << llh.lat << "," << llh.lon << "," << llh.height);
		ROS_INFO_STREAM("Status: " << (unsigned int)llh.flags << ", Hdop: " << driver->hdop << ", NSat: " << driver->num_llh_satellites << ", Heading: " << driver->heading);
...
/*
	void baseline_heading_callback(u16 sender_id, u8 len, u8 msg[], void *context)
	{
		...
		driver->heading = sbp_baseline_heading.heading;
		ROS_INFO("GPS Heading: %d", sbp_baseline_heading.heading);
...
*/
	...
	void imu_raw_callback(u16 sender_id, u8 len, u8 msg[], void *context)
	{
        ...
        imu_msg->linear_acceleration.x = sbp_imu.acc_x;
		imu_msg->linear_acceleration.y = sbp_imu.acc_y;
		imu_msg->linear_acceleration.z = sbp_imu.acc_z;

		imu_msg->angular_velocity.x = sbp_imu.gyr_x;
		imu_msg->angular_velocity.y = sbp_imu.gyr_y;
		imu_msg->angular_velocity.z = sbp_imu.gyr_z;
